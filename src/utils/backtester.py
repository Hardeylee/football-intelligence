...
def build_weighted_profiles(train_files: list) -> dict:
    ...
    for filepath, weight in train_files:
        ...
        for m in matches:
            ...
            # Home team — weighted contribution
            s = stats[home]
            s["weight"] += weight
            ...
            # Away team — weighted contribution
            s = stats[away]
            s["weight"] += weight
...
def run_backtest() -> dict:
    ...
    # Market config: (model_key, odds_key_over, odds_key_under)
    markets = {
        "home_win":     ("B365H",     None),
        "draw":         ("B365D",     None),
        "away_win":     ("B365A",     None),
        "over25":       ("B365>2.5",  None),
        "btts":         (None,        None),  # No B365 BTTS in CSV
        "over35_cards": (None,        None),  # No B365 cards in CSV
    }
